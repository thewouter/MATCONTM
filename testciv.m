% TEST CIV
function [X4, V4, s4, h4, f4] = testciv(mapfile, firstX, firstP) % The input we want to have here but isnt being used, are the hard coded x1 and p1 for each map, as we cannot find these automatically
...
x1 = firstX; %Test values for the Delayed Logistic Map and Adaptive Control Map are [0.444444;0.444444] [1;1;0.38]
p1 = firstP; %Test values for DLM and ACM[1.8; 0.1] [-0.54;1.14;0.1]
...
p3 = p1;
p3(1) = X1(end, s1(2).index); % Set parameters
...
opt = contset(opt, 'MinStepsize', 1.0000e-5);
...
amp = 0.005; %Initial amplitude
[X40, V40] = init_NSm_ICm(mapfile, x4, p4, N, amp, [1, 2], 1);
[X4, V4, s4, h4, f4] = cont(@closedinvariantcurve, X40, V40, opt);