classdef NumericMinipanel < handle
    %NUMERIC Summary of this class goes here
    %   Detailed explanation goes here
    
    properties
        selectedList = struct('coordinates', 1 ,'parameters' , 1,'testfunctions', 0, 'multipliers' , 0 ,'current_stepsize' , 0 , 'user_functions' , 0 , 'npoints' , 0);
        allowedList;
        handleList;
        startdata
    ...
    methods
        function obj = NumericMinipanel(system , curvetype , uiparent ,  varargin)
        ...
        function setValue(obj, key, subkey , value)
            set( obj.handleList.(key).(subkey) , 'String' , num2str(value ,'%0.8g') );
        end
        ...
        function configureLists(obj, system ,curvetype)
            ...
            for i = 1:length(coordlist)
                obj.handleList.coordinates.(coordlist{i}) = [];
            end
            
            paramlist = system.getParameterList();
            obj.handleList.parameters = struct();
            for i = 1:length(paramlist)
                obj.handleList.parameters.(paramlist{i}) = [];
            end

            obj.handleList.multipliers = struct();
            for i = 1:length(coordlist)
                obj.handleList.multipliers.(['mod_' num2str(i)]) = [];
            end
            for i = 1:length(coordlist)
                obj.handleList.multipliers.(['arg_' num2str(i)]) = [];
            end           
            ...
            for i = 1:length(testlist)
                obj.handleList.testfunctions.( testlist{i} ) = [] ;
            end
            ...
            if (~isempty(obj.startdata))
                ...
                for i = 1:ufnr
                      if (  obj.startdata.isValidUserfunction(i) )
                          obj.handleList.user_functions.(obj.startdata.getLabelUserfunction(i)) = [];
                      end 
                end
            end
            
            obj.handleList.npoints = [];
            obj.handleList.current_stepsize = [];
